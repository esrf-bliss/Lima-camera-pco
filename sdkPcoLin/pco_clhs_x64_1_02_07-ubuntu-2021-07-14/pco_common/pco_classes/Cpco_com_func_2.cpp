...
DWORD CPco_com::PCO_GetHWIOSignalDescriptor(WORD SignalNum,char *outbuf,int *size)
{
  ...
  printf("SignalNum = %d %d\n",SignalNum,com.wNumSignal);
...
DWORD CPco_com::PCO_GetColorCorrectionMatrix(char* szCCM,WORD* len)
{
    ...
    if(err==PCO_NOERROR)
    {
     memcpy(szCCM,&resp.szCCM,length);
     *len = length;
    }
    return err;
}
...
DWORD CPco_com::PCO_GetExternalRegister(WORD* wID,BYTE* bData,WORD *len)
{
    ...
    if (err==PCO_NOERROR)
    {
      ...
      memcpy(bData,&resp.bData,length);
      *len=resp.wSize-3*sizeof(WORD)-sizeof(BYTE);
      *wID = resp.wID;
    }
    return err;
...
DWORD CPco_com::PCO_WriteMailbox(WORD wMailboxNo,BYTE* bData,WORD len)
{
    ...
    if(len > sizeof(com.bData))
     len = sizeof(com.bData);
    memcpy(&com.bData,bData,len);
...
DWORD CPco_com::PCO_GetMailboxStatus(WORD* wNumberOfMailboxes,WORD* wMailboxStatus,WORD *len)
{
    ...
    DWORD err=PCO_NOERROR;
    WORD length=sizeof(resp.wMailboxStatus);
     ...
     *wNumberOfMailboxes = resp.wNumberOfMailboxes;
     memcpy(wMailboxStatus,&resp.wMailboxStatus,length);
     *len = sizeof(resp.wMailboxStatus); //in bytes
    }
...
DWORD CPco_com::PCO_SetIntensifiedMCP(WORD wIntensifiedVoltage,DWORD dwIntensifiedPhosphorDecay_us)
{
  ...
  com.wSize=sizeof(com);
  com.wIntensifiedVoltage=wIntensifiedVoltage;
  com.wRsrvd=0;
  com.dwIntensifiedPhosphorDecay_us=dwIntensifiedPhosphorDecay_us;
  com.dwRsrvd1=0;
  com.dwRsrvd2=0;
