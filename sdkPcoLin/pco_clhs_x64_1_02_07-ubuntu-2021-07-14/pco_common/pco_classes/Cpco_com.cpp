...
DWORD CPco_com::test_checksum(unsigned char *buf,int *size)
{
    ...
    if(packet_size>*size)
    {
        writelog(ERROR_M,(PCO_HANDLE)1,"test_checksum size error packet_size %d size %d",packet_size,*size);
        return PCO_ERROR_DRIVER_CHECKSUMERROR | PCO_ERROR_DRIVER_CAMERALINK;
    }
    ...
    if(*(buf+x)!=cks)
    {
        writelog(ERROR_M,(PCO_HANDLE)1,"test_checksum checksum error *(buf+x) 0x%x !=cks 0x%x",*(buf+x),cks);
        return PCO_ERROR_DRIVER_CHECKSUMERROR | PCO_ERROR_DRIVER_CAMERALINK;
    }
