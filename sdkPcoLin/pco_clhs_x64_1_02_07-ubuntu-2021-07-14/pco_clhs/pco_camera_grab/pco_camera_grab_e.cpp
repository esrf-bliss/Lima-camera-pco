...
DWORD grab_expos_wait(CPco_com* camera,CPco_grab_clhs* grabber,int count)
{
 int err,i,timeout,sleep_tim1,sleep_tim2;
 int picnum,buf_nr;
 unsigned int w,h,bp;
 WORD *picbuf[BUFNUM];
 double tim;
...
// WORD triggermode;
 ...
 sleep_tim1=500;
 sleep_tim2=5000;
 printf("enter sleep time 1 in ms [%d]...<CR>: ",sleep_tim1);
 get_number(number,10);
 if(strlen(number))
 {
  sleep_tim1=atoi(number);
 }
 printf("enter sleep time 2 in ms [%d]...<CR>: ",sleep_tim2);
 get_number(number,10);
 if(strlen(number))
 {
  sleep_tim2=atoi(number);
 }

 
 printf("enter exposure time 1 in %s [%d]...<CR>: ",tb[1],expos1);
 get_number(number,10);
 if(strlen(number))
 {
  expos1=atoi(number);
 ...
 printf("enter exposure time 2 in %s [%d]...<CR>: ",tb[1],expos2);
 get_number(number,10);
 if(strlen(number))
 {
  expos2=atoi(number);
 ...
 for(i=0;i<count;i++)
 {
  ...
  printf("Set/Get RecordingState done exp_time %dms\n",exp_time/1000);
  printf("Wait_For_Next_Image timeout %d\n",timeout);
...
//  freq/=tim;
  ...
  std::this_thread::sleep_for(std::chrono::milliseconds(sleep_tim1));
  printf("Sleep % 5d done\n",sleep_tim1);
  ...
  if(err!=PCO_NOERROR)
   break;
  printf("Set exposure time %dms done\n",exp_time/1000); 
...
//15:20:12.318      0.337ms 29458:11960 05 0x0101 Control_Command: done com_in 0x1012  com_out 0x1092 err 0x00000000
  fflush(NULL);
  std::this_thread::sleep_for(std::chrono::milliseconds(sleep_tim2));
  printf("Sleep % 5d done\n",sleep_tim2);
 }
...
DWORD grab_expos_wait1(CPco_com* camera,CPco_grab_clhs* grabber,int count)
{
 int err,i,timeout,sleep_tim1,sleep_tim2;
 int picnum,buf_nr;
 unsigned int w,h,bp;
