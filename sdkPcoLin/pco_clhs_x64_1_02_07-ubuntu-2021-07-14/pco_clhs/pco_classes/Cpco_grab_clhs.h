...
//             |                                                   //
//-----------------------------------------------------------------//
// (c) 2016 -2017 PCO AG * Donaupark 11 *                          //
...
public:
  ...
  ///
  /// \brief Waits until one or more buffers have completely transferred images or are canceled.
  /// \param timeout Sets a timeout in milliseconds. If no buffer has been transferred the function returns with an error.
  /// \return Error code or 0 in case of success
  ///
  DWORD Wait_For_Buffer_from_Queue(int timeout);
  ...
  /// \return Error code or 0 in case of success
  ///
  DWORD Set_Buffer_to_Queue(int start, int end,int synch,void* adr,int len);
  
  DWORD Get_Pending_Buffer();
  
  DWORD Get_Succeeded_Buffer();
