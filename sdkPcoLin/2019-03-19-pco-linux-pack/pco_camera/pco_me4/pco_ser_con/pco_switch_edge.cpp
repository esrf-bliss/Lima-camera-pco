...
//           |            |                                        //
//-----------------------------------------------------------------//
//  1.04     | 24.04.2017 | cl camera on me4                       //
//           |            | use setup functions                    //
//           |            |                                        //
//-----------------------------------------------------------------//
...
int switch_camera(CPco_com_cl_me4* camera,int mode,int cam_num);
...
int main(int argc, char* argv[])
{
  ...
  if((camtype!=CAMERATYPE_PCO_EDGE)&&(camtype!=CAMERATYPE_PCO_EDGE_GL))
  {
...
int switch_camera(CPco_com_cl_me4* cam,int mode,int cam_num)
{
  ...
  err=cam->PCO_GetTransferParameter(&baudrate,sizeof(baudrate));
  if(err!=PCO_NOERROR)
  {
   printf("PCO_GetTransferParameter baudrate error 0x%x\n",err);
   return err;
  }
  
  def_baudrate=9600;
  err=cam->PCO_SetTransferParameter(&def_baudrate,sizeof(def_baudrate));
  if(err!=PCO_NOERROR)
  {
   printf("PCO_SetTransferParameter baudrate 9600 error 0x%x\n",err);
   return err;
  }
  ...
  err=cam->PCO_SetTransferParameter(&baudrate,sizeof(baudrate));
  if(err!=PCO_NOERROR)
  {
   printf("PCO_SetTransferParameter baudrate error 0x%x\n",err);
   return err;
  }
