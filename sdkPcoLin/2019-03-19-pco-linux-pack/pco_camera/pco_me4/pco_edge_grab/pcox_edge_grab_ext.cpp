...
DWORD grab_single(CPco_grab_cl_me4 *grabber,void *picbuf,int pictimeout);
DWORD grab_count(CPco_grab_cl_me4 *grabber,int ima_count,CPCODisp *Cdispwin,CCambuf *Cbuf,int pictimeout);
DWORD grab_loop(CPco_grab_cl_me4 *grabber,int ima_count,CPCODisp *Cdispwin,CCambuf *Cbuf,int pictimeout);
...
CPco_Log mylog("pcox_edge_grab_ext.log");

const char tb[3][3]={"ns","us","ms"};

#define IMAGE_TIMEOUT 30000 //milliseconds
...
int main(int argc, char* argv[])
{
  ...
  int ima_count=10;
  int loop_count=1;
  int PicTimeOut=IMAGE_TIMEOUT; //30 seconds
  WORD act_recstate;
...
//set camera timebase to us
  ...
  err=camera->PCO_SetTriggerMode(TRIGGER_MODE_EXTERNALTRIGGER);
  if(err!=PCO_NOERROR)
...
*/
   ...
   else if(c=='2')
   {
    ...
    {
     if(grab_single(grabber,Cbuf.Get_actadr(),PicTimeOut)==PCO_NOERROR)
      Cdispwin->convert();
    }
    printf("\n");
   }
   else if(c=='3')
   {
    if(act_recstate==0)
     ...
     grab_count(grabber,bufnum,Cdispwin,&Cbuf,PicTimeOut);
    printf("\n");
   }
   else if(c=='4')
   {
    if(act_recstate==0)
     ...
     grab_loop(grabber,ima_count,Cdispwin,&Cbuf,PicTimeOut);
    printf("\n");
   }
  }
...
DWORD grab_single(CPco_grab_cl_me4 *grabber,void *picbuf)
{
  return grab_single(grabber,picbuf,IMAGE_TIMEOUT);
}
...
DWORD grab_count(CPco_grab_cl_me4 *grabber,int ima_count,CPCODisp *Cdispwin,CCambuf *Cbuf)
{
  return grab_count(grabber,ima_count,Cdispwin,Cbuf,IMAGE_TIMEOUT);
}
...
DWORD grab_count(CPco_grab_cl_me4 *grabber,int ima_count,CPCODisp *Cdispwin,CCambuf *Cbuf,int pictimeout)
{
 ...
 if(err==PCO_NOERROR)
 {
  ...
  err=grabber->Wait_For_Next_Image(&picnum,pictimeout);
 ...
 if(err==PCO_NOERROR)
 {
  ...
  fflush(stdout);
  err=grabber->Wait_For_Images(ima_count,ima_count*pictimeout/1000);
  if(err!=PCO_NOERROR)
  {
 ...
 if(err==PCO_NOERROR)
 {
  ...
  {
   err1=grabber->Check_DMA_Length(x);
   if(err1!=PCO_NOERROR)
...
DWORD grab_loop(CPco_grab_cl_me4 *grabber,int ima_count,CPCODisp *Cdispwin,CCambuf *Cbuf)
{
  return grab_loop(grabber,ima_count,Cdispwin,Cbuf,IMAGE_TIMEOUT);
...
//for testing only lost images can also be seen at summary output
  ...
  {
   numoff++;
//   printf("\ngrab_loop Error number: camera %d grabber %d offset now %d",currentnum-firstnum+1,picnum,numoff);
  }
  if(ima_count<=20)
   printf("\n");
  fflush(stdout);
