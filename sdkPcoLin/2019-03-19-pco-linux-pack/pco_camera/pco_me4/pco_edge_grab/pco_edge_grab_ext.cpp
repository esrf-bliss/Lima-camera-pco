...
DWORD grab_single(CPco_grab_cl_me4 *grabber,char * filename,int pictimeout);


DWORD grab_count(CPco_grab_cl_me4 *grabber,int ima_count);
DWORD grab_loop(CPco_grab_cl_me4 *grabber,int ima_count);

DWORD grab_count(CPco_grab_cl_me4 *grabber,int ima_count,int pictimeout);
DWORD grab_loop(CPco_grab_cl_me4 *grabber,int ima_count,int pictimeout);
...
CPco_Log mylog("pco_edge_grab_ext.log");

const char tb[3][3]={"ns","us","ms"};

const char tmode[4][12]={"auto","software","ext. frame","ext. expos"};
const char tpol[4][12]={"high level","low level","rising","falling"};

#define IMAGE_TIMEOUT 30000 //30 seconds
...
int main(int argc, char* argv[])
{
  ...
  DWORD signalstatus,sensorimacount;
  WORD act_trigmode;
  WORD act_trigpolarity;
...
//set camera timebase to us
  ...
  act_trigmode=TRIGGER_MODE_EXTERNALTRIGGER;
  err=camera->PCO_SetTriggerMode(act_trigmode);
  if(err!=PCO_NOERROR)
...
/*
  ...
  FilterSetting=0;
  Selected=0;
  printf("PCO_SetHWIOSignal(0,%d,%d,%d,%d,%d)\n",Enabled,Type,Polarity,FilterSetting,Selected);
  printf("Input 0: Enabled  %d\n Type %d\n Polarity %d\n Filter %d\n Selected %d\n",Enabled,Type,Polarity,FilterSetting,Selected);
  err=camera->PCO_SetHWIOSignal(0,Enabled,Type,Polarity,FilterSetting,Selected);
  if(err!=PCO_NOERROR)
...
//prepare Camera for recording
  ...
  err=camera->PCO_GetHWIOSignal(0,&Enabled,&Type,&Polarity,&FilterSetting,&Selected);
  if(err!=PCO_NOERROR)
   printf("PCO_GetHWIOSignal(0,) Error 0x%x\n",err);
  else
   printf("PCO_GetHWIOSignal(0,) returned:\n Enabled  %d\n Type %d\n Polarity %d\n Filter %d\n Selected %d\n",Enabled,Type,Polarity,FilterSetting,Selected);
...
//transfer dataformat must be changed depending on pixelrate and horizontal resolution
  ...
  while(c!='x')
  {
   ...
   camera->PCO_GetSensorSignalStatus(&signalstatus,&sensorimacount);
   camera->PCO_GetTriggerMode(&act_trigmode);
   camera->PCO_GetHWIOSignal(0,&Enabled,&Type,&Polarity,&FilterSetting,&Selected);
   ...
   printf("Sensor signalstatus 0x%x  imacount %d\n",signalstatus,sensorimacount);
   printf("\n");
   ...
   printf("                                 Clockfrequency  %d\n",clpar.ClockFrequency);
   printf("                                 Baudrate        %d\n",clpar.baudrate);
   printf("m to set trigger mode            Trigger_mode    %s\n",tmode[act_trigmode]);
   printf("n to set trigger polarity        Polarity        %s\n",tpol[act_trigpolarity]);
...
*/
   ...
   else if(c=='m')
   {
    if(act_recstate==1)
     camera->PCO_SetRecordingState(0);
    printf("enter new triggermode  ...<CR>: ");
    get_number(number,2);
    if(strlen(number))
     act_trigmode=atoi(number);
    err=camera->PCO_SetTriggerMode(act_trigmode);
    if(err!=PCO_NOERROR)
     printf("\nSet_TriggerMode failed\n");
    else
     printf("\nSet_TriggerMode done\n");
   ...
   else if(c=='n')
   {
    if(act_recstate==1)
     camera->PCO_SetRecordingState(0);
    printf("enter new trigger polarity  ...<CR>: ");
    printf("high level   = 1\n");
    printf("low level    = 2\n");
    printf("rising       = 4\n");
    printf("falling      = 8\n");
   ...
   {
     WORD trigdone;
     err=camera->PCO_ForceTrigger(&trigdone);
     if(err!=PCO_NOERROR)
      printf("\nForceTrigger failed\n");
     else
      printf("\nForceTrigger done 0x%x \n",trigdone);
   ...
   else if(c=='2')
   {
    ...
    else
    {
     ...
     if(strlen(filename))
      grab_single(grabber,filename,PicTimeOut);
     else
      grab_single(grabber,NULL,PicTimeOut);
    }
    printf("\n");
   }
   else if(c=='3')
   {
    if(act_recstate==0)
     ...
     grab_count(grabber,bufnum,PicTimeOut);
    printf("\n");
   }
   else if(c=='4')
   {
    if(act_recstate==0)
     ...
     grab_loop(grabber,ima_count,PicTimeOut);
    printf("\n");
   }
  }
  ...
  err=camera->PCO_SetTriggerMode(TRIGGER_MODE_AUTOTRIGGER);
  if(err!=PCO_NOERROR)
   printf("PCO_SetTriggerMode() Error 0x%x\n",err);
...
{
  return grab_single(grabber,NULL,IMAGE_TIMEOUT);
}


DWORD grab_single(CPco_grab_cl_me4 *grabber,int pictimeout)
{
  return grab_single(grabber,NULL,pictimeout);
}

DWORD grab_single(CPco_grab_cl_me4 *grabber,char* filename)
{
  return grab_single(grabber,filename,IMAGE_TIMEOUT);
}
...
DWORD grab_single(CPco_grab_cl_me4 *grabber,char* filename,int pictimeout)
{
 ...
 if(err!=PCO_NOERROR)
  printf("\ngrab_single Set_Grabber_Timeout error 0x%x\n",err);

 printf("grab_single  Wait for external trigger\n");
...
DWORD grab_single(CPco_grab_cl_me4 *grabber,char* filename,int pictimeout)
{
 ...
 {
  printf("grab_single  Wait for external trigger\n");
  err=grabber->Wait_For_Image(&picnum,pictimeout);
  if(err!=PCO_NOERROR)
 ...
 if(grabber->Stop_Acquire()!=PCO_NOERROR)
  printf("\ngrab_single Stop_Acquire error\n");
...
DWORD grab_count(CPco_grab_cl_me4 *grabber,int ima_count)
{
 return grab_count(grabber,ima_count,IMAGE_TIMEOUT);
}
...
DWORD grab_count(CPco_grab_cl_me4 *grabber,int ima_count,int pictimeout)
{
 ...
 if(err==PCO_NOERROR)
 {
  printf("grab_count  Wait for external trigger\n");
  err=grabber->Wait_For_Next_Image(&picnum,pictimeout/1000);
 ...
 if(err==PCO_NOERROR)
 {
  printf("wait until %d images are grabbed waittime %ds",ima_count,ima_count*pictimeout/1000);
  fflush(stdout);
  err=grabber->Wait_For_Images(ima_count,pictimeout/1000*ima_count);
  if(err!=PCO_NOERROR)
  {
...
DWORD grab_loop(CPco_grab_cl_me4 *grabber,int ima_count)
{
  return grab_loop(grabber,ima_count,IMAGE_TIMEOUT);
}
...
DWORD grab_loop(CPco_grab_cl_me4 *grabber,int ima_count,int pictimeout)
{
 ...
 do
 {
  printf("Wait for external trigger image %06d ",imanum+1);
  fflush(stdout);
  err=grabber->Wait_For_Next_Image(&picnum,pictimeout/1000);
  if(err!=PCO_NOERROR)
  ...
  if(err!=PCO_NOERROR)
  {
   printf("\ngrab_loop Error break loop at image number %06d",imanum+1);
   break;
  }
  imanum++;
...
//  currentnum=image_nr_from_timestamp(linebuf,0);
  ...
  printf(" done currentnum %06d %06d\r",currentnum,currentnum-firstnum+1);
