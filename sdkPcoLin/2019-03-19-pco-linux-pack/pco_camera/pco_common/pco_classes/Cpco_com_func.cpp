...
DWORD CPco_com::PCO_GetCameraSetup(WORD *setup_id, DWORD *setup_flag, WORD *length)
{
  *setup_id=0;
  return PCO_GetCameraSetup(*setup_id,setup_flag,length);
}

DWORD CPco_com::PCO_GetCameraSetup(WORD setup_id, DWORD *setup_flag, WORD *length)
{
  DWORD err=PCO_NOERROR;
  SC2_Get_Camera_Setup com;
  SC2_Get_Camera_Setup_Response resp;
  WORD len;

  if(*length < NUMSETUPFLAGS)
    len = *length;
  else
    len = NUMSETUPFLAGS;

  com.wCode=GET_CAMERA_SETUP;
  com.wSize=sizeof(SC2_Get_Camera_Setup);
  com.wType=setup_id;
  err=Control_Command(&com,sizeof(com),&resp,sizeof(resp));
  if(setup_flag)
    memcpy(setup_flag,resp.dwSetupFlags,len*sizeof(DWORD));
  else 
   *length=sizeof(resp.dwSetupFlags)/sizeof(DWORD);
  return err;
}

DWORD CPco_com::PCO_SetCameraSetup(WORD setup_id, DWORD *setup_flag,WORD length)
{
  DWORD err=PCO_NOERROR;
  ...
  com.wType=setup_id;
  memcpy(com.dwSetupFlags,setup_flag,len*sizeof(DWORD));
  err=Control_Command(&com,sizeof(com),&resp,sizeof(resp));
  return err;
}
...
DWORD CPco_com::PCO_RebootCamera_Wait(DWORD connect_time)
{
  ...
  DWORD time_done;
  DWORD err;
  DWORD actbaudrate,oldbaudrate;

  err=PCO_GetTransferParameter(&oldbaudrate,sizeof(DWORD));
  writelog(INIT_M,hdriver,"Reboot() save oldbaudrate %d",oldbaudrate);

  actbaudrate=9600;
  err=PCO_SetTransferParameter(&actbaudrate,sizeof(DWORD));
  if(err!=0)
   writelog(ERROR_M,hdriver,"set_baudrate:%d error:%d",actbaudrate,err);
  ...
  if(err==PCO_NOERROR)
  {
   err =PCO_SetTransferParameter(&oldbaudrate,sizeof(DWORD));
   if(err!=0)
    writelog(ERROR_M,hdriver,"set_baudrate:%d error:%d",actbaudrate,err);
  }
