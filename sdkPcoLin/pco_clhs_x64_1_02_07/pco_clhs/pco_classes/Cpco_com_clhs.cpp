...
{
  this->writelog(INIT_M,hdriver,"~CPco_com_clhs: before Close_Cam()");
  Close_Cam();
}
...
DWORD CPco_com_clhs::scan_camera()
{
  ...
  for(int i=0;i<5;i++)
  {
   this->writelog(INIT_M,hdriver,"scan_camera: scan with GET_CAMERA_TYPE %d",i);
   ...
   if(err==PCO_NOERROR)
   {
    this->writelog(INIT_M,hdriver,"scan_camera: successful");
    break;
   }
  }
  return err;
...
DWORD CPco_com_clhs::Open_Cam_Ext(DWORD num,SC2_OpenStruct *open ATTRIBUTE_UNUSED)
{
  ...
  this->writelog(INIT_M, (PCO_HANDLE)1,"%s: Common Version %02d.%02d.%02d",__FUNCTION__,COMMON_VERSION,COMMON_SUBVERSION,COMMON_SUBAGE);
  this->writelog(INIT_M, (PCO_HANDLE)1,"%s: CLHS libpcocom_clhs Version %02d.%02d.%02d",__FUNCTION__,VERSION,SUBVERSION,SUBAGE);
  ...
  {
   this->writelog(ERROR_M,(PCO_HANDLE)1,"%s: No more entries left return NODRIVER",__FUNCTION__);
   return PCO_ERROR_DRIVER_NODRIVER | PCO_ERROR_DRIVER_CAMERALINK;
  }

  if(hdriver)
  {
   this->writelog(ERROR_M,(PCO_HANDLE)1,"%s: camera is already connected",__FUNCTION__);
   return PCO_ERROR_DRIVER_NODRIVER | PCO_ERROR_DRIVER_CAMERALINK;
  }
  ...
  if(Cclhs_cam==NULL)
  {
   ...
   if(err!=PCO_NOERROR)
   {
    this->writelog(ERROR_M,hdriver,"%s: Cclhs_cam->Open failed 0x%x",__FUNCTION__,err);
    Cclhs_cam->Close();
    delete Cclhs_cam;
    Cclhs_cam=NULL;
    return err;
  ...
  if(err==PCO_NOERROR)
  {
   if(sem_init(&sMutex, 0, 1) == -1)
    this->writelog(PROCESS_M, (PCO_HANDLE)1,"%s: Could not get semaphore sMutex",__FUNCTION__);
 ...
 //get camera descriptor to get maximal size of ccd
   err=scan_camera();
   if(err!=PCO_NOERROR)
    this->writelog(ERROR_M,hdriver,"%s: Control command failed with 0x%08x, no camera connected",__FUNCTION__,err);
  }
  ...
  if(err==PCO_NOERROR)
  {
   err=get_description();
   if(err!=PCO_NOERROR)
    this->writelog(ERROR_M,hdriver,"%s: get_desription() failed with 0x%08x",__FUNCTION__,err);
  }
  ...
  if(err==PCO_NOERROR)
  {
   err=get_firmwarerev();
   if(err!=PCO_NOERROR)
    this->writelog(ERROR_M,hdriver,"%s: get_firmwarerev() failed with 0x%08x",__FUNCTION__,err);
  }
  ...
  if(err==PCO_NOERROR)
  {
   ...
   this->writelog(INIT_M,hdriver,"%s: wait done set camera time to %02d.%02d.%04d %02d:%02d:%02d.%06d",__FUNCTION__,
             st.tm_mday,st.tm_mon+1,st.tm_year+1900,st.tm_hour,st.tm_min,st.tm_sec,tv.tv_usec);
#endif

   err=PCO_SetDateTime(&st);
   if(err!=PCO_NOERROR)
    this->writelog(ERROR_M,hdriver,"%s: PCO_SetDateTime() failed with 0x%08x",__FUNCTION__,err);
#endif
  }
...
DWORD CPco_com_clhs::Close_Cam()
{
  if(hdriver==(PCO_HANDLE)NULL)
  {
   this->writelog(INIT_M,hdriver,"%s: driver was closed before",__FUNCTION__);
   return PCO_NOERROR;
  }

  if(Cclhs_cam)
  {
   this->writelog(INIT_M,hdriver,"%s: close Cclhs_cam",__FUNCTION__);
   Cclhs_cam->Close();
   delete Cclhs_cam;
   Cclhs_cam=NULL;

   this->writelog(INIT_M,hdriver, "%s: Close mutex 0x%x",__FUNCTION__, sMutex);
   sem_close(&sMutex);
   sem_destroy(&sMutex);
   Sleep_ms(100);
...
//wait for semaphore
  ...
  {
   if(errno == ETIMEDOUT)
    this->writelog(ERROR_M,hdriver,"%s: mutex timed out errno %d",__FUNCTION__,errno);
   else
    this->writelog(ERROR_M,hdriver,"%s: some other error errno %d ",__FUNCTION__,errno);
...
/*  
  {
   ...
   this->writelog(COMMAND_M,hdriver,"%s: start com_in %s timeout %d",__FUNCTION__,buf,tab_timeout.command);
  }
*/  
  this->writelog(COMMAND_M,hdriver,"%s: start com_in 0x%04x timeout %d",__FUNCTION__,com_in,tab_timeout.command);
  ...
  if(err!=PCO_NOERROR)
  {
   this->writelog(ERROR_M,hdriver,"%s: write_mem failed with 0x%x",__FUNCTION__,err);
   goto clhs_com_end;
  }
...
//data after telegram length is random therefore we set it to 0
  ...
  if(err != 0)
  {
   this->writelog(ERROR_M,hdriver,"%s: read_mem failed with 0x%x",__FUNCTION__,err);
   goto clhs_com_end;
  }
  ...
  if(reg.size>40)
  {
   ...
   this->writelog(COMMAND_M,hdriver,"%s: read answer again with size %d",__FUNCTION__,reg.size);
   err=Cclhs_cam->read_cam(reg.adr,&reg.size,buffer);
   if(err != 0)
   {
    this->writelog(ERROR_M,hdriver,"%s: read_mem failed with 0x%x",__FUNCTION__,err);
  ...
  {
   this->writelog(ERROR_M,hdriver,"%s: comin  0x%04x != comout&0xFF3F 0x%04x",__FUNCTION__,com_in,com_out&0xFF3F);
  ...
  if((com_out&RESPONSE_ERROR_CODE)==RESPONSE_ERROR_CODE)
  {
   ...
   if((err&0xC000FFFF)==PCO_ERROR_FIRMWARE_NOT_SUPPORTED)
    this->writelog(INTERNAL_1_M,hdriver,"%s: com 0x%x FIRMWARE_NOT_SUPPORTED",__FUNCTION__,com_in);
   else
    this->writelog(ERROR_M,hdriver,"%s: com 0x%x RESPONSE_ERROR_CODE error 0x%x",__FUNCTION__,com_in,err);
  }
  ...
  if(err==PCO_NOERROR)
  {
   if(com_out!=(com_in|RESPONSE_OK_CODE))
   {
    ...
    this->writelog(ERROR_M,hdriver,"%s: Data error com_out 0x%04x should be 0x%04x",__FUNCTION__,com_out,com_in|RESPONSE_OK_CODE);
   }
  }
  ...
  this->writelog(INTERNAL_1_M,hdriver,"%s: before test_checksum read=0x%04x size %d",__FUNCTION__,com_out,size);
...
clhs_com_end:
  sem_post(&sMutex);
  this->writelog(COMMAND_M,hdriver,"%s: done com_in 0x%04x  com_out 0x%04x data 0x%04x err 0x%08x",__FUNCTION__,com_in,com_out,data,err);
  return err;
}
